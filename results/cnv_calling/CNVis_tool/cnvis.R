...
# Ensure there are 37 unique samples
if (length(unique_sample_ids) != 35) {
  stop("The number of unique sample IDs is not 35. Please check your data.")
}
...
# Load masked regions (e.g., centromeres, telomeres)
masked_regions <- rtracklayer::import("../../../datasets/masked_regions/hg38-blacklist.v2.bed")

# Find overlaps and exclude CNVs in masked regions
overlaps <- findOverlaps(cnv_granges, masked_regions)
cnv_granges <- cnv_granges[-queryHits(overlaps)]

## Exclude overlapping CNVs between case samples and controls
# Separate control and case samples
control_samples = annotations$sample[annotations$diagnosis=='Control']
tumor_samples = annotations$sample[annotations$diagnosis!='Control']

control_granges <- cnv_granges[cnv_granges$sample %in% control_samples]
tumor_granges <- cnv_granges[cnv_granges$sample %in% tumor_samples]

# Find overlaps between control and case CNVs
overlaps_with_controls <- findOverlaps(tumor_granges, control_granges)

# Exclude CNVs in cases that overlap with controls
filtered_tumor_granges <- tumor_granges[-queryHits(overlaps_with_controls)]

cnv_filtered = as.data.table(filtered_tumor_granges)
colnames(cnv_filtered)[1] = 'chrom'
...
Heatmap(
  ...
  name = "Log2 Ratio",
  col = heatmap_colors,
  right_annotation = ha,border = T,
  cluster_rows = T,
  #cluster_columns = as.dendrogram(hc_samples),
  show_row_names = T,
  show_column_names = T,
...
manhattan_plot <- generate_manhattan_plot(cnv_merged_filtered)
...
cnv_summary <- cnv_merged_filtered[, .(
  mean_log2_ratio = mean(log2_ratio_normalized, na.rm = TRUE),
  sd_log2_ratio = sd(log2_ratio_normalized, na.rm = TRUE)
...
chrom_heatmap_colors <- colorRamp2(c(-3, 0, 3), c("blue", "white", "red"))
...
significant_cnvs <- cnv_uniq[abs(cnv_uniq$log2_ratio) > 0.3, ]
...
p <- ggbio::autoplot(
  ...
  geom_text(
    data = as.data.frame(cnv_granges_sig[abs(cnv_granges_sig$log2_ratio) > 0.3, ]), # Filter significant CNVs
    aes(
...
samples = unique(cnv_uniq$sample)

for (sample in samples) {
  sample_data = subset(cnv_uniq, sample == sample)
